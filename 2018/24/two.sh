...
num_units=0
declare -a u_side u_count u_hp u_atype u_apow u_init u_prop1 u_prop2 u_prop3 u_mod1 u_mod2 i_mod3 u_busy orig_apow
addprop() {
  if [ -z "${u_prop1[n]}" ] ; then
    u_mod1[n]=$1
    u_prop1[n]=$2
...
reset="${u_count[@]@A};totA=$totA;totD=$totD"
...
for ((;;)) ; do
  ...
  if [ -z "$win_boost" ] ; then
    ((boost *= 2))
  elif ((win_boost==fail_boost+1)) ; then
    break
  else
    ((boost=(win_boost+fail_boost)/2))
  fi
done
